...
fetch() {
  ...
  if [ $rc -ne 0 ]; then
    ...
    printf "\n" >&2
    info "This is likely due to hsearch not yet supporting your configuration." >&2
    ...
    info "${BOLD}${UNDERLINE}https://github.com/grepinsight/hsearch/issues/new/${NO_COLOR}\n" >&2
    exit $rc
  fi
}
...
fetch_and_unpack() {
  ...
  if [ "${EXT}" = "tar.gz" ]; then
    ...
    info "bug report at https://github.com/hsearch/hsearch/issues"
    exit 1
  fi
}
...
install() {
  ...
  if test_writeable "${BIN_DIR}"; then
    sudo=""
    msg="Installing hsearch, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Installing hsearch as root, please wait…"
  fi
  info "$msg"
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/grepinsight/history_search/releases"
fi
...
if [ "${ARCH}" = "i386" ]; then
  error "i386 builds are not yet available for hsearch\n"
  info "If you would like to see a build for your configuration,"
  info "please create an issue requesting a build for ${MAGENTA}${ARCH}-${PLATFORM}${NO_COLOR}:"
  info "${BOLD}${UNDERLINE}https://github.com/hsearch/hsearch/issues/new/${NO_COLOR}\n"
  exit 1
fi
...
EXT=tar.gz
URL="${BASE_URL}/latest/download/hsearch-${ARCH}-${PLATFORM}.${EXT}"
info "Tarball URL: ${UNDERLINE}${BLUE}${URL}${NO_COLOR}"
confirm "Install hsearch ${GREEN}latest${NO_COLOR} to ${BOLD}${GREEN}${BIN_DIR}${NO_COLOR}?"
check_bin_dir "${BIN_DIR}"

install
complete "hsearch installed"
